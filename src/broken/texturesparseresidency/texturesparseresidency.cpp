...
#include <rendering/heightmap.hpp>
...
struct VirtualTexturePage {
    ...
    void allocate(const vk::MemoryRequirements& memoryRequirements) {
        if (imageMemoryBind.memory) {
            // std::cout << "Page " << index << " already allocated" << std::endl;
            return;
        };

        allocation = vks::Allocation::allocatePages(memoryRequirements);
        allocationInfo = vks::Allocation::getAllocationInfo(allocation);
    ...
    void release(const vk::Device& device) {
        if (imageMemoryBind.memory) {
            vks::Allocation::freePages(allocation);
...
public:
    ...
    virtual void getEnabledFeatures() override {
        ...
        const auto& features = context.deviceInfo.features.core10;
        auto& enabledFeatures = context.enabledFeatures.core10;
        // Device features to be enabled for this example
        enabledFeatures.shaderResourceResidency = features.shaderResourceResidency;
        enabledFeatures.shaderResourceMinLod = features.shaderResourceMinLod;
        enabledFeatures.sparseBinding = features.sparseBinding;
        enabledFeatures.sparseResidencyImage2D = features.sparseResidencyImage2D;
        if (!enabledFeatures.sparseResidencyImage2D || !enabledFeatures.sparseBinding) {
    ...
    void prepareSparseTexture(uint32_t width, uint32_t height, uint32_t layerCount, vk::Format format) {
        ...
        vks::Loader::get().withPrimaryCommandBuffer([&](const vk::CommandBuffer& commandBuffer) {
            vk::ImageSubresourceRange range{ vk::ImageAspectFlagBits::eColor, 0, texture.mipLevels, 0, 1 };
            vks::util::setImageLayout(commandBuffer, texture.image, vk::ImageLayout::eUndefined, vk::ImageLayout::eReadOnlyOptimal, range);
        });
        ...
        if (sparseImageMemoryReqs.size > context.deviceInfo.properties.core10.limits.sparseAddressSpaceSize) {
        ...
        // populate the mip tail
        {
            ...
            // Issue blit commands
            if (!imageBlits.empty()) {
                vk::ImageSubresourceRange range{ vk::ImageAspectFlagBits::eColor, (uint32_t)texture.mipTailStart, texture.mipLevels - texture.mipTailStart, 0,
                                                 1 };
                vks::Loader::get().withPrimaryCommandBuffer([&](const vk::CommandBuffer& copyCmd) {
                    const vk::ArrayProxy<const vk::ImageBlit> ap(imageBlits);
                    vks::util::setImageLayout(copyCmd, texture.image, vk::ImageLayout::eReadOnlyOptimal, vk::ImageLayout::eTransferDstOptimal, range);
                    copyCmd.blitImage(                                                //
                        textures.source.image, vk::ImageLayout::eTransferSrcOptimal,  //
                        texture.image, vk::ImageLayout::eTransferDstOptimal,          //
                        ap, vk::Filter::eLinear);
                    vks::util::setImageLayout(copyCmd, texture.image, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eReadOnlyOptimal, range);
                });
            }
        }
        ...
        texture.descriptor.imageLayout = vk::ImageLayout::eReadOnlyOptimal;
    ...
    void loadAssets() override {
        textures.source.loadFromFile(getAssetPath() + "textures/ground_dry_bc3_unorm.ktx", vk::Format::eBc3UnormBlock, vk::ImageUsageFlagBits::eTransferSrc,
                                     vk::ImageLayout::eTransferSrcOptimal);
        // Generate a terrain quad patch for feeding to the tessellation control shader
        heightMap.loadFromFile(getAssetPath() + "textures/terrain_heightmap_r16.ktx", 128, glm::vec3(2.0f, 48.0f, 2.0f), vkx::HeightMap::topologyTriangles);
    }
    ...
    void preparePipelines() {
        ...
        pipelineBuilder.renderingCreateInfo.colorAttachmentCount = 1;
        pipelineBuilder.renderingCreateInfo.pColorAttachmentFormats = &swapChain.surfaceFormat.format;
        pipelineBuilder.renderingCreateInfo.depthAttachmentFormat = deviceInfo.supportedDepthFormat;
        pipelineBuilder.renderingCreateInfo.stencilAttachmentFormat = deviceInfo.supportedDepthFormat;
        ...
        pipelineBuilder.loadShader(vkx::shaders::texturesparseresidency::sparseresidency::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::texturesparseresidency::sparseresidency::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepare() override {
        ...
        if (!deviceInfo.features.core10.sparseResidencyImage2D) {
    ...
    void fillVirtualTexture(int32_t& mipLevel) {
            ...
            if ((page.mipLevel == mipLevel) && /*(rndDist(rndEngine) < 0.5f) &&*/ !page.imageMemoryBind.memory) {
                ...
                for (uint32_t x = 0; x < scale; x++) {
                    for (uint32_t y = 0; y < scale; y++) {
                        ...
                        blit.srcOffsets[0] = vk::Offset3D{ 0, 0, 0 };
                        blit.srcOffsets[1] =
                            vk::Offset3D{ static_cast<int32_t>(textures.source.extent.width), static_cast<int32_t>(textures.source.extent.height), 1 };
                        // Dest
                        blit.dstSubresource.aspectMask = vk::ImageAspectFlagBits::eColor;
                        blit.dstSubresource.baseArrayLayer = 0;
                        blit.dstSubresource.layerCount = 1;
        ...
        if (imageBlits.size() > 0) {
            ...
            context.withPrimaryCommandBuffer([&](const vk::CommandBuffer& copyCmd) {
                context.setImageLayout(copyCmd, texture.image, vk::ImageLayout::eReadOnlyOptimal, vk::ImageLayout::eTransferDstOptimal, range);
                copyCmd.blitImage(                                                //
                    textures.source.image, vk::ImageLayout::eTransferSrcOptimal,  //
                    texture.image, vk::ImageLayout::eTransferDstOptimal,          //
                    imageBlits, vk::Filter::eLinear);
                context.setImageLayout(copyCmd, texture.image, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eReadOnlyOptimal, range);
            });
