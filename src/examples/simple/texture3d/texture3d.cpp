...
#include <shaders/texture3d/texture3d.frag.inl>
#include <shaders/texture3d/texture3d.vert.inl>
...
public:
    ...
    vks::texture::Texture texture;
    vk::Format textureFormat{ vk::Format::eR8Unorm };
    ...
    void prepareNoiseTexture(uint32_t width, uint32_t height, uint32_t depth) {
        ...
        auto formatProperties = context.physicalDevice.getFormatProperties(textureFormat);
        ...
        uint32_t maxImageDimension3D(context.deviceInfo.properties.core10.limits.maxImageDimension3D);
        ...
        {
            vks::Image::Builder builder{ textureSize };
            builder.imageCreateInfo.imageType = vk::ImageType::e3D;
            builder.withFormat(textureFormat);
            builder.withUsage(vk::ImageUsageFlagBits::eSampled | vk::ImageUsageFlagBits::eTransferDst);
            texture.build(builder, vk::ImageViewType::e3D);
        }
    }
...
#pragma omp parallel for
        for (int32_t z = 0; z < (int32_t)textureSize.depth; z++) {
...
#define FRACTAL
        ...
        vks::Buffer stagingBuffer = loader.createStagingBuffer(data);

        loader.withPrimaryCommandBuffer([&](const vk::CommandBuffer& copyCmd) {
            // Image barrier for optimal image
            // Optimal image will be used as destination for the copy, so we must transfer from our
            // initial undefined image layout to the transfer destination layout
            vks::util::setImageLayout(copyCmd, texture.image, vks::util::ImageTransitionState::UNDEFINED, vks::util::ImageTransitionState::TRANSFER_DST);
            ...
            copyCmd.copyBufferToImage(stagingBuffer.buffer, texture.image.image, vk::ImageLayout::eTransferDstOptimal, bufferCopyRegion);

            // Change texture image layout to shader read after all mip levels have been copied
            vks::util::setImageLayout(copyCmd, texture.image, vks::util::ImageTransitionState::TRANSFER_DST, vks::util::ImageTransitionState::SAMPLED);
        });
    ...
    void setupDescriptorSet() {
        ...
        auto texDesc = texture.makeDescriptor(defaultSampler);
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets{
            { descriptorSet, 0, 0, 1, vk::DescriptorType::eUniformBuffer, nullptr, &uniformBufferVS.descriptor },
            { descriptorSet, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &texDesc },
        };
    ...
    void preparePipelines() {
        ...
        auto& vertexInputState = pipelineBuilder.vertexInputState;
        // Binding description
        ...
        pipelineBuilder.loadShader(vkx::shaders::texture3d::texture3d::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::texture3d::texture3d::frag, vk::ShaderStageFlagBits::eFragment);
