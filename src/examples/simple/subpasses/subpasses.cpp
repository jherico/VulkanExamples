...
#include <shaders/subpasses/composition.frag.inl>
#include <shaders/subpasses/composition.vert.inl>
#include <shaders/subpasses/gbuffer.frag.inl>
#include <shaders/subpasses/gbuffer.vert.inl>
#include <shaders/subpasses/transparent.frag.inl>
#include <shaders/subpasses/transparent.vert.inl>
...
public:
    ...
    struct Attachment {
        ...
        void destroy() {
            image.destroy();
            vks::Context::get().device.destroy(view);
            view = nullptr;
        }
    };
    ...
    struct Attachments {
        Attachment position, normal, albedo;
    } attachments;
    ...
    void getEnabledFeatures() override {
        ...
        enabledFeatures.core10.samplerAnisotropy = deviceFeatures.core10.samplerAnisotropy;
        // Enable texture compression
        enabledFeatures.core10.textureCompressionBC = deviceFeatures.core10.textureCompressionBC;
        enabledFeatures.core10.textureCompressionASTC_LDR = deviceFeatures.core10.textureCompressionASTC_LDR;
        enabledFeatures.core10.textureCompressionETC2 = deviceFeatures.core10.textureCompressionETC2;
    }

    // Create a frame buffer attachment
    void createAttachment(vk::Format format, vk::ImageUsageFlags usage, Attachment& attachment) {
        ...
        imageView.image = attachment.image.image;
    ...
    void setupFrameBuffer() {
        ...
        for (uint32_t i = 0; i < framebuffers.size(); i++) {
            ...
            attachmentsDescs[4] = depthStencilView;
    ...
    void setupRenderPass() {
        ...
        // Color attachment
        attachmentsDescs[0].format = defaultColorFormat;
        ...
        // Position
        attachmentsDescs[1].format = this->attachments.position.image.createInfo.format;
        ...
        attachmentsDescs[1].finalLayout = vk::ImageLayout::eAttachmentOptimal;
        // Normals
        attachmentsDescs[2].format = this->attachments.normal.image.createInfo.format;
        ...
        attachmentsDescs[2].finalLayout = vk::ImageLayout::eAttachmentOptimal;
        // Albedo
        attachmentsDescs[3].format = this->attachments.albedo.image.createInfo.format;
        ...
        attachmentsDescs[3].finalLayout = vk::ImageLayout::eAttachmentOptimal;
        // Depth attachment
        attachmentsDescs[4].format = defaultDepthStencilFormat;
        ...
        attachmentsDescs[4].finalLayout = vk::ImageLayout::eAttachmentOptimal;
        ...
        colorReferences[0] = { 0, vk::ImageLayout::eAttachmentOptimal };
        colorReferences[1] = { 1, vk::ImageLayout::eAttachmentOptimal };
        colorReferences[2] = { 2, vk::ImageLayout::eAttachmentOptimal };
        colorReferences[3] = { 3, vk::ImageLayout::eAttachmentOptimal };
        vk::AttachmentReference depthReference{ 4, vk::ImageLayout::eAttachmentOptimal };
        ...
        inputReferences[0] = { 1, vk::ImageLayout::eReadOnlyOptimal };
        inputReferences[1] = { 2, vk::ImageLayout::eReadOnlyOptimal };
        inputReferences[2] = { 3, vk::ImageLayout::eReadOnlyOptimal };
        ...
        // ----------------------------------------------------------------------------------------
        colorReference = { 0, vk::ImageLayout::eAttachmentOptimal };
    ...
    void updateCommandBufferPreDraw(const vk::CommandBuffer& commandBuffer) {
    ...
    void updateCommandBufferPostDraw(const vk::CommandBuffer& commandBuffer) { commandBuffer.endRenderPass(); }
    ...
    void loadAssets() {
        models.scene.loadFromFile(getAssetPath() + "models/samplebuilding.dae", vertexLayout, 1.0f);
        models.transparent.loadFromFile(getAssetPath() + "models/samplebuilding_glass.dae", vertexLayout, 1.0f);
        // Textures
        if (deviceInfo.features.core10.textureCompressionBC) {
            textures.glass.loadFromFile(getAssetPath() + "textures/colored_glass_bc3_unorm.ktx", vk::Format::eBc3UnormBlock);
        } else if (deviceInfo.features.core10.textureCompressionASTC_LDR) {
            textures.glass.loadFromFile(getAssetPath() + "textures/colored_glass_astc_8x8_unorm.ktx", vk::Format::eAstc8x8UnormBlock);
        } else if (deviceInfo.features.core10.textureCompressionETC2) {
            textures.glass.loadFromFile(getAssetPath() + "textures/colored_glass_etc2_unorm.ktx", vk::Format::eEtc2R8G8B8A8UnormBlock);
        } else {
    ...
    void setupDescriptorSet() {
        ...
        vk::DescriptorImageInfo texDescriptorPosition{ nullptr, attachments.position.view, vk::ImageLayout::eReadOnlyOptimal };
        vk::DescriptorImageInfo texDescriptorNormal{ nullptr, attachments.normal.view, vk::ImageLayout::eReadOnlyOptimal };
        vk::DescriptorImageInfo texDescriptorAlbedo{ nullptr, attachments.albedo.view, vk::ImageLayout::eReadOnlyOptimal };
        vk::DescriptorImageInfo glassTex{ defaultSampler, textures.glass.imageView, vk::ImageLayout::eReadOnlyOptimal };
        ...
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets{
            ...
            vk::WriteDescriptorSet{ descriptorSets.transparent, 2, 0, 1, vk::DescriptorType::eCombinedImageSampler, &glassTex },
        };

        for (auto& writeDescriptorSet : writeDescriptorSets) {
            auto x = writeDescriptorSet;
    ...
    void preparePipelines() {
        // Deferred pass
        vks::pipelines::GraphicsPipelineBuilder builder{ device, pipelineLayouts.offscreen };
        ...
        builder.vertexInputState.erase(3);
        builder.loadShader(vkx::shaders::subpasses::gbuffer::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::subpasses::gbuffer::frag, vk::ShaderStageFlagBits::eFragment);
        ...
        builder.loadShader(vkx::shaders::subpasses::composition::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::subpasses::composition::frag, vk::ShaderStageFlagBits::eFragment);
        ...
        vertexLayout.appendVertexLayout(builder.vertexInputState);
        builder.vertexInputState.erase(2);
        builder.layout = pipelineLayouts.transparent;
        builder.subpass = 2;
        builder.loadShader(vkx::shaders::subpasses::transparent::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::subpasses::transparent::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        uniformBuffers.GBuffer = loader.createUniformBuffer(uboGBuffer);

        // Deferred fragment shader
        uniformBuffers.lights = loader.createUniformBuffer(uboLights);
    ...
    void updateUniformBufferDeferredMatrices() {
        ...
        uniformBuffers.GBuffer.copy(uboGBuffer);
    }
    ...
    void updateUniformBufferDeferredLights() {
        ...
        uniformBuffers.lights.copy(uboLights);
    }
