...
#include <shaders/texturearray/instancing.frag.inl>
#include <shaders/texturearray/instancing.vert.inl>
...
public:
    ...
    void loadTextures() {
        const auto& features = context.deviceInfo.features;
        vk::Format format;
        std::string filename;
        if (features.core10.textureCompressionBC) {
            filename = "texturearray_bc3_unorm.ktx";
            format = vk::Format::eBc3UnormBlock;
        } else if (features.core10.textureCompressionASTC_LDR) {
            filename = "texturearray_astc_8x8_unorm.ktx";
            format = vk::Format::eAstc8x8UnormBlock;
        } else if (features.core10.textureCompressionETC2) {
        ...
        textureArray.loadFromFile(getAssetPath() + "textures/" + filename, format);
    }
    ...
    void updateDrawCommandBuffer(const vk::CommandBuffer& cmdBuffer) override {
        ...
        cmdBuffer.drawIndexed(meshes.quad.indexCount, textureArray.image.createInfo.arrayLayers, 0, 0, 0);
    }
...
#undef dim
    ...
    void setupDescriptorSet() {
        ...
        auto texDesc = textureArray.makeDescriptor(defaultSampler);
    ...
    void preparePipelines() {
        ...
        using namespace vkx::shaders::texturearray;
        pipelineBuilder.loadShader(instancing::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(instancing::frag, vk::ShaderStageFlagBits::eFragment);
        pipelines.solid = pipelineBuilder.create(context.pipelineCache);
    }

    void prepareUniformBuffers() {
        // Vertex shader uniform buffer block
        uniformData.vertexShader = loader.createUniformBuffer(uboVS);

        // Array indices and model matrices are fixed
        float offset = -1.5f;
        float center = (textureArray.image.createInfo.arrayLayers * offset) / 2;
        for (uint32_t i = 0; i < textureArray.image.createInfo.arrayLayers; i++) {
