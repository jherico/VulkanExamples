...
#include <shaders/descriptorsets/cube.frag.inl>
#include <shaders/descriptorsets/cube.vert.inl>
...
public:
    ...
    void getEnabledFeatures() override {
        ...
        if (context.deviceInfo.features.core10.samplerAnisotropy) {
            context.enabledFeatures.core10.samplerAnisotropy = VK_TRUE;
        };
    }
    ...
    void loadAssets() override {
        models.cube.loadFromFile(getAssetPath() + "models/cube.dae", vertexLayout, 1.0f);
        cubes[0].texture.loadFromFile(getAssetPath() + "textures/crate01_color_height_rgba.ktx");
        cubes[1].texture.loadFromFile(getAssetPath() + "textures/crate02_color_height_rgba.ktx");
    }
	...
	*/
    void setupDescriptors() {
		...
		*/
        ...
        auto max_sets = static_cast<uint32_t>(cubes.size());

        // Create the global descriptor pool
        descriptorPool = device.createDescriptorPool({ {}, max_sets, descriptorPoolSizes });
		...
		*/
        ...
        std::vector<vk::DescriptorImageInfo> cubeDescriptors;
        for (auto& cube : cubes) {
            auto& cubeDescriptor = cubeDescriptors.emplace_back(cube.texture.makeDescriptor(defaultSampler));
        }

        for (int i = 0; i < cubes.size(); ++i) {
            auto& cube = cubes[i];
            auto& cubeDescriptor = cubeDescriptors[i];
			...
			*/
            ...
            writeDescriptorSets.push_back({ cube.descriptorSet, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &cubeDescriptor });
        }
    ...
    void preparePipelines() {
		...
		*/
        ...
        auto pipelineBuilder = vks::pipelines::GraphicsPipelineBuilder{ device, pipelineLayout };
        ...
        pipelineBuilder.loadShader(vkx::shaders::descriptorsets::cube::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::descriptorsets::cube::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        for (auto& cube : cubes) {
            cube.uniformBuffer = loader.createUniformBuffer(cube.matrices);
        }
    ...
    void updateUniformBuffers() {
        ...
        for (auto& cube : cubes) {
            ...
            cube.uniformBuffer.copy(cube.matrices);
        }
    }
