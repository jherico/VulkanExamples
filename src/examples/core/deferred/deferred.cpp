...
#include <shaders/deferred/debug.frag.inl>
#include <shaders/deferred/debug.vert.inl>
#include <shaders/deferred/deferred.frag.inl>
#include <shaders/deferred/deferred.vert.inl>
#include <shaders/deferred/mrt.frag.inl>
#include <shaders/deferred/mrt.vert.inl>
...
public:
    ...
    void loadAssets() override {
        textures.colorMap.loadFromFile(getAssetPath() + "models/armor/colormap.ktx", vk::Format::eBc3UnormBlock);
        meshes.example.loadFromFile(getAssetPath() + "models/armor/armor.dae", vertexLayout, 1.0f);
    }
    ...
    void setupDescriptorSet() {
        ...
        vk::DescriptorImageInfo texDescriptorPosition{ offscreen.sampler, offscreen.colorTargets[0].imageView, vk::ImageLayout::eGeneral };
        vk::DescriptorImageInfo texDescriptorNormal{ offscreen.sampler, offscreen.colorTargets[1].imageView, vk::ImageLayout::eGeneral };
        vk::DescriptorImageInfo texDescriptorAlbedo{ offscreen.sampler, offscreen.colorTargets[2].imageView, vk::ImageLayout::eGeneral };
        ...
        // Offscreen (scene)
        auto colorMapDesc = textures.colorMap.makeDescriptor(defaultSampler);
        ...
        std::vector<vk::WriteDescriptorSet> offscreenWriteDescriptorSets{
            ...
            { descriptorSets.offscreen, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &colorMapDesc },
        };
        device.updateDescriptorSets(offscreenWriteDescriptorSets, nullptr);
    }

    void preparePipelines() {
        vks::pipelines::GraphicsPipelineBuilder pipelineBuilder{ device, pipelineLayouts.deferred };
        auto& targets = offscreen.colorTargets;
        ...
        pipelineBuilder.loadShader(vkx::shaders::deferred::deferred::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::deferred::deferred::frag, vk::ShaderStageFlagBits::eFragment);
        pipelines.deferred = pipelineBuilder.create(context.pipelineCache);

        // Debug display pipeline
        pipelineBuilder.destroyShaderModules();
        pipelineBuilder.loadShader(vkx::shaders::deferred::debug::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::deferred::debug::frag, vk::ShaderStageFlagBits::eFragment);
        pipelines.debug = pipelineBuilder.create(context.pipelineCache);

        // Offscreen pipeline
        pipelineBuilder.destroyShaderModules();
        // for the offscreen pass we care about the depth test
        pipelineBuilder.depthStencilState = true;
        pipelineBuilder.dynamicRendering({ targets[0].image.createInfo.format, targets[1].image.createInfo.format, targets[2].image.createInfo.format },
                                         defaultDepthStencilFormat, defaultDepthStencilFormat);
        pipelineBuilder.loadShader(vkx::shaders::deferred::mrt::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::deferred::mrt::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        uniformData.vsFullScreen = loader.createUniformBuffer(uboVS);
        // Deferred vertex shader
        uniformData.vsOffscreen = loader.createUniformBuffer(uboOffscreenVS);
        // Deferred fragment shader
        uniformData.fsLights = loader.createUniformBuffer(uboFragmentLights);
    ...
    void prepareOffscreen() override {
        auto builder = vkx::offscreen::Builder(TEX_DIM);
        builder
            // position
