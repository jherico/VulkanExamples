...
#include <shaders/dynamicuniformbuffer/base.frag.inl>
#include <shaders/dynamicuniformbuffer/base.vert.inl>
...
public:
    ...
    void setupDescriptorSet() {
        ...
        auto dynamicDescriptor = uniformBuffers.dynamic.getDescriptor(0, sizeof(glm::mat4));
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets{
            { descriptorSet, 0, 0, 1, vk::DescriptorType::eUniformBuffer, nullptr, &uniformBuffers.view.descriptor },
            { descriptorSet, 1, 0, 1, vk::DescriptorType::eUniformBufferDynamic, nullptr, &dynamicDescriptor },
        };
    ...
    void preparePipelines() {
        ...
        pipelineBuilder.loadShader(vkx::shaders::dynamicuniformbuffer::base::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::dynamicuniformbuffer::base::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        size_t minUboAlignment = deviceInfo.properties.core10.limits.minUniformBufferOffsetAlignment;
        ...
        uboDataDynamic.model.resize(OBJECT_INSTANCES);
        ...
        uniformBuffers.view = loader.createUniformBuffer(uboVS);

        // Uniform buffer object with per-object matrices
        uniformBuffers.dynamic = vks::Buffer::Builder{ bufferSize }
                                     .withBufferUsage(vk::BufferUsageFlagBits::eUniformBuffer)
                                     .withAllocCreateFlags(VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT | VMA_ALLOCATION_CREATE_HOST_ACCESS_SEQUENTIAL_WRITE_BIT |
    ...
    void updateUniformBuffers() {
        ...
        uniformBuffers.view.copy(uboVS);
    }
    ...
    void updateDynamicUniformBuffer(bool force = false) {
        ...
        for (uint32_t x = 0; x < dim; x++) {
            for (uint32_t y = 0; y < dim; y++) {
                for (uint32_t z = 0; z < dim; z++) {
                    ...
                    // Aligned offset
                    glm::mat4& modelMat = uboDataDynamic.model[index];
        ...
        animationTimer = 0.0f;
        if (dynamicAlignment != sizeof(glm::mat4)) {
            vk::ArrayProxy<const glm::mat4> ap(uboDataDynamic.model);
            uniformBuffers.dynamic.copyWithStride(ap, dynamicAlignment);
        } else {
            uniformBuffers.dynamic.copy(uboDataDynamic.model);
        }
