...
#include <shaders/shadowmapping/offscreen.frag.inl>
#include <shaders/shadowmapping/offscreen.vert.inl>
#include <shaders/shadowmapping/quad.frag.inl>
#include <shaders/shadowmapping/quad.vert.inl>
#include <shaders/shadowmapping/scene.frag.inl>
#include <shaders/shadowmapping/scene.vert.inl>
...
public:
    ...
    void buildOffscreenCommandBuffer() override {
        ...
        if (offscreen.cmdBuffer) {
            recycler.trashCommandBuffers(offscreen.queue.pool, offscreen.cmdBuffer);
        }
        offscreen.cmdBuffer = offscreen.queue.createCommandBuffer();
    ...
    void loadAssets() override { meshes.scene.loadFromFile(getAssetPath() + "models/vulkanscene_shadow.dae", vertexLayout, 4.0f); }
...
#undef QUAD_COLOR_NORMAL
    ...
    void setupDescriptorSets() {
        ...
        vk::DescriptorImageInfo texDescriptor{ offscreen.sampler, offscreen.depthTarget.imageView, vk::ImageLayout::eReadOnlyOptimal };
        ...
        texDescriptor.imageView = offscreen.depthTarget.imageView;
    ...
    void preparePipelines() {
        ...
        vks::pipelines::GraphicsPipelineBuilder pipelineCreator{ device, pipelineLayouts.quad };
        ...
        pipelineCreator.loadShader(vkx::shaders::shadowmapping::quad::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineCreator.loadShader(vkx::shaders::shadowmapping::quad::frag, vk::ShaderStageFlagBits::eFragment);
        pipelines.quad = pipelineCreator.create(context.pipelineCache);

        // 3D scene
        pipelineCreator.destroyShaderModules();
        pipelineCreator.loadShader(vkx::shaders::shadowmapping::scene::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineCreator.loadShader(vkx::shaders::shadowmapping::scene::frag, vk::ShaderStageFlagBits::eFragment);
        pipelines.scene = pipelineCreator.create(context.pipelineCache);

        // Offscreen pipeline
        pipelineCreator.destroyShaderModules();
        pipelineCreator.loadShader(vkx::shaders::shadowmapping::offscreen::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineCreator.loadShader(vkx::shaders::shadowmapping::offscreen::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        uniformDataVS = loader.createUniformBuffer(uboVSscene);
        // Offsvreen vertex shader uniform buffer block
        uniformDataOffscreenVS = loader.createUniformBuffer(uboOffscreenVS);
    ...
    void prepareOffscreen() override {
        ...
        builder.withDepthFormat(vk::Format::eD16Unorm, vk::ImageUsageFlagBits::eSampled);
