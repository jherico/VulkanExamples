...
#include <shaders/shadowmappingomni/cubemapdisplay.frag.inl>
#include <shaders/shadowmappingomni/cubemapdisplay.vert.inl>
#include <shaders/shadowmappingomni/offscreen.frag.inl>
#include <shaders/shadowmappingomni/offscreen.vert.inl>
#include <shaders/shadowmappingomni/scene.frag.inl>
#include <shaders/shadowmappingomni/scene.vert.inl>
...
public:
    ...
    vk::Sampler shadowCubeMapSampler;
    ...
    ~VulkanExample() {
        ...
        // Pipelibes
        device.destroy(pipelines.scene);
        device.destroy(pipelines.offscreen);
        device.destroy(pipelines.cubeMap);
    ...
    void prepareCubeMap() {
        ...
        shadowCubeMap.create(imageCreateInfo);
        shadowCubeMapView = shadowCubeMap.createView(vk::ImageViewType::eCube);
        loader.forceImageLayout(graphicsQueue, shadowCubeMap, vk::ImageLayout::eReadOnlyOptimal);
        ...
        shadowCubeMapSampler = device.createSampler(sampler);
    }
    ...
    void updateCubeFace(uint32_t faceIndex) {
        ...
        setImageLayout(offscreen.cmdBuffer, offscreen.colorTargets[0].image, ImageTransitionState::UNDEFINED, ImageTransitionState::COLOR_ATTACHMENT);
        ...
        copyRegion.extent = shadowCubeMap.createInfo.extent;

        setImageLayout(offscreen.cmdBuffer, offscreen.colorTargets[0].image, ImageTransitionState::COLOR_ATTACHMENT, ImageTransitionState::TRANSFER_SRC);

        // Put image copy into command buffer
        offscreen.cmdBuffer.copyImage(offscreen.colorTargets[0].image.image, vk::ImageLayout::eTransferSrcOptimal, shadowCubeMap.image,
    ...
    void buildOffscreenCommandBuffer() override {
        ...
        // rendering
        if (!cmdBuffer) {
            cmdBuffer = graphicsQueue.createCommandBuffer();
        }
        ...
        // Change image layout for all cubemap faces to transfer destination
        using namespace vks::util;
        setImageLayout(cmdBuffer, shadowCubeMap, ImageTransitionState::SAMPLED, ImageTransitionState::TRANSFER_DST);
        for (uint32_t face = 0; face < 6; ++face) {
            updateCubeFace(face);
        }
        // Change image layout for all cubemap faces to shader read after they have been copied
        setImageLayout(cmdBuffer, shadowCubeMap, ImageTransitionState::TRANSFER_DST, ImageTransitionState::SAMPLED);
        offscreen.cmdBuffer.end();
    }

    // void updateCommandBufferPreDraw(const vk::CommandBuffer& cmdBuffer) override {
    //     // Change image layout for all cubemap faces to transfer destination
    //     context.setImageLayout(cmdBuffer, shadowCubeMap.image, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eReadOnlyOptimal, CUBEMAP_RANGE);
    // }
    ...
    void loadAssets() override {
        meshes.skybox.loadFromFile(getAssetPath() + "models/cube.obj", vertexLayout, 2.0f);
        meshes.scene.loadFromFile(getAssetPath() + "models/shadowscene_fire.dae", vertexLayout, 2.0f);
    }
    ...
    void setupDescriptorSets() {
        ...
        vk::DescriptorImageInfo texDescriptor{ shadowCubeMapSampler, shadowCubeMapView, vk::ImageLayout::eReadOnlyOptimal };
    ...
    void preparePipelines() {
        // 3D scene pipeline
        vks::pipelines::GraphicsPipelineBuilder builder{ device, pipelineLayouts.scene };
        ...
        builder.loadShader(vkx::shaders::shadowmappingomni::scene::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::shadowmappingomni::scene::frag, vk::ShaderStageFlagBits::eFragment);
        ...
        builder.destroyShaderModules();
        builder.loadShader(vkx::shaders::shadowmappingomni::cubemapdisplay::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::shadowmappingomni::cubemapdisplay::frag, vk::ShaderStageFlagBits::eFragment);
        builder.rasterizationState.cullMode = vk::CullModeFlagBits::eFront;
        pipelines.cubeMap = builder.create(context.pipelineCache);

        // Offscreen pipeline
        builder.destroyShaderModules();
        builder.loadShader(vkx::shaders::shadowmappingomni::offscreen::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::shadowmappingomni::offscreen::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        uniformData.offscreen = loader.createUniformBuffer(uboOffscreenVS);
        // 3D scene
        uniformData.scene = loader.createUniformBuffer(uboVSscene);
    ...
    void prepareOffscreen() override {
        ...
        builder.appendColorFormat(vk::Format::eR32Sfloat, vk::ImageUsageFlagBits::eTransferSrc);
