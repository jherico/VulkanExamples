...
#include <shaders/distancefieldfonts/bitmap.frag.inl>
#include <shaders/distancefieldfonts/bitmap.vert.inl>
#include <shaders/distancefieldfonts/sdf.frag.inl>
#include <shaders/distancefieldfonts/sdf.vert.inl>
...
public:
    ...
    void loadAssets() override {
        textures.fontSDF.loadFromFile(getAssetPath() + "textures/font_sdf_rgba.ktx", vk::Format::eR8G8B8A8Unorm);
        textures.fontBitmap.loadFromFile(getAssetPath() + "textures/font_bitmap_rgba.ktx", vk::Format::eR8G8B8A8Unorm);

        std::string fileName = getAssetPath() + "font.fnt";

        vks::file::withBinaryFileContents(fileName, [&](auto span) {
            imemstream istream((const char*)span.data(), span.size());
            assert(istream.good());
            while (!istream.eof()) {
                std::string line;
    ...
    void generateText(std::string text) {
        ...
        float w = (float)textures.fontSDF.image.createInfo.extent.width;
    ...
    void setupDescriptorSet() {
        ...
        auto sdfDesc = textures.fontSDF.makeDescriptor(defaultSampler);
        auto bmdDesc = textures.fontBitmap.makeDescriptor(defaultSampler);
        ...
        std::vector<vk::WriteDescriptorSet> writeDescriptorSets{
            ...
            vk::WriteDescriptorSet{ descriptorSets.sdf, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &sdfDesc },
            ...
            // Binding 1 : Fragment shader texture sampler
            vk::WriteDescriptorSet{ descriptorSets.bitmap, 1, 0, 1, vk::DescriptorType::eCombinedImageSampler, &bmdDesc },
        };
    ...
    void preparePipelines() {
        ...
        pipelineBuilder.colorBlendState.blendAttachmentStates[0] = { VK_TRUE, BF::eOne, BF::eOneMinusSrcAlpha, BO::eAdd, BF::eOne, BF::eZero, BO::eAdd };
        vertexLayout.appendVertexLayout(pipelineBuilder.vertexInputState);
        pipelineBuilder.loadShader(vkx::shaders::distancefieldfonts::sdf::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::distancefieldfonts::sdf::frag, vk::ShaderStageFlagBits::eFragment);
        ...
        pipelineBuilder.loadShader(vkx::shaders::distancefieldfonts::bitmap::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::distancefieldfonts::bitmap::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void prepareUniformBuffers() {
        ...
        uniformData.vs = loader.createUniformBuffer(uboVS);

        // Fragment sahder uniform buffer block
        // Contains font rendering parameters
        uniformData.fs = loader.createUniformBuffer(uboFS);
