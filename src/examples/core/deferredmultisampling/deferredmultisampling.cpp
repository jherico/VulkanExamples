...
#include <shaders/deferredmultisampling/debug.frag.inl>
#include <shaders/deferredmultisampling/debug.vert.inl>
#include <shaders/deferredmultisampling/deferred.frag.inl>
#include <shaders/deferredmultisampling/deferred.vert.inl>
#include <shaders/deferredmultisampling/mrt.frag.inl>
#include <shaders/deferredmultisampling/mrt.vert.inl>
...
public:
    ...
    vk::Sampler offscreenColorSampler;
    ...
    struct Attachment {
        ...
        void create(const vk::ImageCreateInfo& createInfo) {
            image.create(createInfo, VMA_ALLOCATION_CREATE_DEDICATED_MEMORY_BIT);
            view = image.createView(vk::ImageViewType::e2D);
        }
    ...
    void getEnabledFeatures() override {
        ...
        auto& deviceFeatures = context.deviceInfo.features.core10;
    ...
    void prepareOffscreen() override {
        auto builder = vkx::offscreen::Builder(size);
        builder
            // position
            .appendColorFormat(vk::Format::eR16G16B16A16Sfloat, vk::ImageUsageFlagBits::eSampled)
            // normal
            .appendColorFormat(vk::Format::eR16G16B16A16Sfloat, vk::ImageUsageFlagBits::eSampled)
            // color
            .appendColorFormat(vk::Format::eR8G8B8A8Unorm, vk::ImageUsageFlagBits::eSampled)
        ...
        offscreenColorSampler = device.createSampler(sampler);
    }

    // Build command buffer for rendering the scene to the offscreen frame buffer attachments
    void buildOffscreenCommandBuffer() {
        if (offscreen.cmdBuffer) {
            recycler.trashCommandBuffers(graphicsQueue.pool, offscreen.cmdBuffer);
            offscreen.cmdBuffer = graphicsQueue.createCommandBuffer();
        }

        offscreen.cmdBuffer.begin({ vk::CommandBufferUsageFlagBits::eSimultaneousUse });
        offscreen.setLayout(offscreen.cmdBuffer, vks::util::ImageTransitionState::COLOR_ATTACHMENT);
        ...
        offscreen.cmdBuffer.bindPipeline(vk::PipelineBindPoint::eGraphics, useSampleShading ? pipelines.offscreenSampleShading : pipelines.offscreen);
        offscreen.cmdBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayouts.offscreen, 0, 1, &descriptorSets.floor, 0, NULL);
        offscreen.cmdBuffer.bindVertexBuffers(0, { models.floor.vertices.buffer }, { 0 });
        ...
        offscreen.cmdBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pipelineLayouts.offscreen, 0, 1, &descriptorSets.model, 0, NULL);
        offscreen.cmdBuffer.bindVertexBuffers(0, { models.model.vertices.buffer }, { 0 });
        ...
        offscreen.setLayout(offscreen.cmdBuffer, vks::util::ImageTransitionState::SAMPLED);
    ...
    void loadAssets() override {
        ...
        models.floor.loadFromFile(getAssetPath() + "models/openbox.dae", vertexLayout, modelCreateInfo);
        ...
        textures.model.colorMap.loadFromFile(getAssetPath() + "models/armor/color" + texFormatSuffix + ".ktx", texFormat);
        textures.model.normalMap.loadFromFile(getAssetPath() + "models/armor/normal" + texFormatSuffix + ".ktx", texFormat);
        textures.floor.colorMap.loadFromFile(getAssetPath() + "textures/stonefloor02_color" + texFormatSuffix + ".ktx", texFormat);
        textures.floor.normalMap.loadFromFile(getAssetPath() + "textures/stonefloor02_normal" + texFormatSuffix + ".ktx", texFormat);
    }
    ...
    void setupDescriptorSet() {
        ...
        auto& position = offscreen.colorTargets[0];
        auto& normal = offscreen.colorTargets[1];
        auto& albedo = offscreen.colorTargets[2];
        // Image descriptors for the offscreen color attachments
        vk::DescriptorImageInfo texDescriptorPosition{ offscreenColorSampler, position.imageView, vk::ImageLayout::eReadOnlyOptimal };
        vk::DescriptorImageInfo texDescriptorNormal{ offscreenColorSampler, normal.imageView, vk::ImageLayout::eReadOnlyOptimal };
        vk::DescriptorImageInfo texDescriptorAlbedo{ offscreenColorSampler, albedo.imageView, vk::ImageLayout::eReadOnlyOptimal };

        // Image descriptors for the offscreen color attachments
        vk::DescriptorImageInfo modelColorMapDesc = textures.model.colorMap.makeDescriptor(defaultSampler);
        vk::DescriptorImageInfo modelNormalMapDesc = textures.model.normalMap.makeDescriptor(defaultSampler);
        vk::DescriptorImageInfo floorColorMapDesc = textures.floor.colorMap.makeDescriptor(defaultSampler);
        vk::DescriptorImageInfo floorNormalMapDesc = textures.floor.normalMap.makeDescriptor(defaultSampler);

        std::vector<vk::WriteDescriptorSet> writeDescriptorSets = {
            // Binding 0 : Vertex shader uniform buffer
            { descriptorSet, 0, 0, vk::DescriptorType::eUniformBuffer, nullptr, uniformBuffers.vsFullScreen.descriptor },
            // Binding 1 : Position texture target
            { descriptorSet, 1, 0, vk::DescriptorType::eCombinedImageSampler, texDescriptorPosition },
            // Binding 2 : Normals texture target
            { descriptorSet, 2, 0, vk::DescriptorType::eCombinedImageSampler, texDescriptorNormal },
            // Binding 3 : Albedo texture target
            { descriptorSet, 3, 0, vk::DescriptorType::eCombinedImageSampler, texDescriptorAlbedo },
            // Binding 4 : Fragment shader uniform buffer
            { descriptorSet, 4, 0, vk::DescriptorType::eUniformBuffer, nullptr, uniformBuffers.fsLights.descriptor },
            // Binding 0: Vertex shader uniform buffer
            { descriptorSets.model, 0, 0, vk::DescriptorType::eUniformBuffer, nullptr, uniformBuffers.vsOffscreen.descriptor },
            // Binding 1: Color map
            { descriptorSets.model, 1, 0, vk::DescriptorType::eCombinedImageSampler, modelColorMapDesc },
            // Binding 2: Normal map
            { descriptorSets.model, 2, 0, vk::DescriptorType::eCombinedImageSampler, modelNormalMapDesc },
            // Binding 0: Vertex shader uniform buffer
            { descriptorSets.floor, 0, 0, vk::DescriptorType::eUniformBuffer, nullptr, uniformBuffers.vsOffscreen.descriptor },
            // Binding 1: Color map
            { descriptorSets.floor, 1, 0, vk::DescriptorType::eCombinedImageSampler, floorColorMapDesc },
            // Binding 2: Normal map
            { descriptorSets.floor, 2, 0, vk::DescriptorType::eCombinedImageSampler, floorNormalMapDesc },
        };
        device.updateDescriptorSets(writeDescriptorSets, nullptr);
    }

    void preparePipelines() {
        auto& position = offscreen.colorTargets[0].image;
        auto& normal = offscreen.colorTargets[1].image;
        auto& albedo = offscreen.colorTargets[2].image;
        ...
        // With MSAA
        builder.loadShader(vkx::shaders::deferredmultisampling::deferred::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::deferredmultisampling::deferred::frag, vk::ShaderStageFlagBits::eFragment);
        ...
        builder.loadShader(vkx::shaders::deferredmultisampling::debug::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::deferredmultisampling::debug::frag, vk::ShaderStageFlagBits::eFragment);
        ...
        builder.dynamicRendering(
            {
                position.createInfo.format,
                normal.createInfo.format,
                albedo.createInfo.format,
            },
            defaultDepthStencilFormat, defaultDepthStencilFormat);
        vertexLayout.appendVertexLayout(builder.vertexInputState);
        builder.loadShader(vkx::shaders::deferredmultisampling::mrt::vert, vk::ShaderStageFlagBits::eVertex);
        builder.loadShader(vkx::shaders::deferredmultisampling::mrt::frag, vk::ShaderStageFlagBits::eFragment);
    ...
    void OnUpdateUIOverlay() override {
        if (ui.header("Settings")) {
            ...
            if (context.deviceInfo.features.core10.sampleRateShading) {
