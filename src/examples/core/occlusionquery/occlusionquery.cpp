...
#include <shaders/occlusionquery/mesh.frag.inl>
#include <shaders/occlusionquery/mesh.vert.inl>
#include <shaders/occlusionquery/occluder.frag.inl>
#include <shaders/occlusionquery/occluder.vert.inl>
#include <shaders/occlusionquery/simple.frag.inl>
#include <shaders/occlusionquery/simple.vert.inl>
...
public:
    ...
    ~VulkanExample() {
        ...
        device.destroy(pipelines.occluder);
        device.destroy(pipelines.simple);
    ...
    void setupQueryResultBuffer() {
        ...
        queryResult = vks::Buffer::Builder{ bufSize }
                          .withBufferUsage(vk::BufferUsageFlagBits::eUniformBuffer | vk::BufferUsageFlagBits::eTransferDst)
                          .withAllocCreateFlags(VMA_ALLOCATION_CREATE_HOST_ACCESS_RANDOM_BIT | VMA_ALLOCATION_CREATE_MAPPED_BIT)
                          .build();
    ...
    void loadMeshes() {
        meshes.plane.loadFromFile(getAssetPath() + "models/plane_z.3ds", vertexLayout, 0.4f);
        meshes.teapot.loadFromFile(getAssetPath() + "models/teapot.3ds", vertexLayout, 0.3f);
        meshes.sphere.loadFromFile(getAssetPath() + "models/sphere.3ds", vertexLayout, 0.3f);
    }
    ...
    void preparePipelines() {
        ...
        pipelineBuilder.loadShader(vkx::shaders::occlusionquery::mesh::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::occlusionquery::mesh::frag, vk::ShaderStageFlagBits::eFragment);
        // Solid rendering pipeline
        ...
        pipelineBuilder.loadShader(vkx::shaders::occlusionquery::simple::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::occlusionquery::simple::frag, vk::ShaderStageFlagBits::eFragment);
        pipelineBuilder.rasterizationState.cullMode = vk::CullModeFlagBits::eNone;
        pipelines.simple = pipelineBuilder.create(context.pipelineCache);
        pipelineBuilder.destroyShaderModules();

        // Visual pipeline for the occluder
        pipelineBuilder.loadShader(vkx::shaders::occlusionquery::occluder::vert, vk::ShaderStageFlagBits::eVertex);
        pipelineBuilder.loadShader(vkx::shaders::occlusionquery::occluder::frag, vk::ShaderStageFlagBits::eFragment);
        // Enable blending
    ...
    void prepareUniformBuffers() {
        ...
        // Teapot
        uniformData.teapot = loader.createUniformBuffer(uboVS);
        // Sphere
        uniformData.sphere = loader.createUniformBuffer(uboVS);
